...
public class Prisoner implements Cloneable, Comparable<Prisoner>
{
	...
	// if normal move use normal strategy 
		...
		//Return action (C or D) from that index in genome
     ...
     */
    public static Prisoner getRand(int seed)
    {
	BitSet ra;
	Random rand;
	if (seed==-1)
     ...
     */
    public static Prisoner[][] getRand(int num_players,int num_players2, int seed)
    {
	BitSet ra;
	Random rand;
	if (seed==-1)
