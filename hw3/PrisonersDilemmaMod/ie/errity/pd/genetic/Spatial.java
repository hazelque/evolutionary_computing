...
public class Spatial extends JPanel
{
	...
	if(rand.nextDouble() <= rules.getCrossP())
	    Offspring = Genetic.crossover(world[ parent1[0] ][ parent1[1] ].getStrat(),world[parent2[0]][parent2[1]].getStrat(), rand);	
	else //CLONE
	    {
	...
	//MUTATION
	Offspring[0] = Genetic.mutate(Offspring[0], rules.getMutateP(), rand);
	Offspring[1] = Genetic.mutate(Offspring[1], rules.getMutateP(), rand);
